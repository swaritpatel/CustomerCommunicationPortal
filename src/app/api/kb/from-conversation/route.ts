        ...
        messages: [
          {
            role: "system",
            content:
              "Create concise customer-support knowledge base articles from resolved or recurring issues. Do not invent policies, refunds, dates, amounts, or guarantees. Return only valid JSON.",
          },
          {
            role: "user",
            content: [
...
async function POSTHandler(request: Request) {
  try {
    ...
    if (conversation.messages.length === 0) {
      return NextResponse.json({ error: "Conversation has no messages to learn from" }, { status: 400 });
    }
